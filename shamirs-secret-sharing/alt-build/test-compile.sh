...
# copy dependencies
#
mvn dependency:copy-dependencies -DoutputDirectory=${PROJECT_LIBS_DIR} -DincludeScope=test
...
SOURCES=$(find ./src/test -name *.scala -type f -printf "%p ")
...
mkdir --parents ./target/test-classes

${JAVA} -Dpath.separator=${CLASSPATH_SEPARATOR} --class-path ${COMPILER_CLASSPATH} -Dscala.expandjavacp=true -Dscala.usejavacp=true \
dotty.tools.MainGenericCompiler -d ./target/test-classes \
-classpath \
./target/shamirs-secret-sharing-${SHAMIR_VERSION}.jar${CLASSPATH_SEPARATOR}${PROJECT_CLASSPATH} \
-deprecation \
-verbose \
-encoding \
utf-8 \
